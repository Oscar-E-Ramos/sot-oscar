...
from dynamic_graph.sot.dyninv.meta_task_dyn_passing_point import goto6dPP
...
class FeetFollowCapturePoint(object):
    def __init__(self, taskFoot, contactFoot, taskCapturePoint, sot):
        ...
        ''' Variables to control the error (limits) '''
        self.errorz = 0.005
        self.errorx = 0.015
    ...
    def update(self):
        if( self.taskCP.CPOutsideLimits ):
            ...
            if( self.footUP ):
                ...
                ''' Check the error between the task and the CP in (x,y) '''
                self.taskF.feature.error.recompute(self.sot.solution.time)
                if((abs(self.taskF.feature.error.value[2]) < self.errorz) and
                   (abs(self.taskF.feature.error.value[0]) < self.errorx)):
                    self.footUP = False
                ...
                ''' Check the error between the task and the CP in x,y '''
                self.taskF.feature.error.recompute(self.sot.solution.time)
                #print self.taskF.feature.error.value[2]
                if((self.taskCP.task.capturePoint.value[0] < self.xmax) and
                   ...
                   (abs((self.taskF.feature.position.value)[2][3] -
                        (self.taskF.featureDes.position.value)[2][3]) < self.errorz)):
...
class FeetToPredictedCapturePoint(object):
    def __init__(self, taskFoot, contactFoot, taskCapturePoint, sot):
        self.taskF   = taskFoot
        self.contact = contactFoot
        self.taskCP  = taskCapturePoint
    ...
    def update(self):
        if( self.taskCP.CPOutsideLimits ):
            ...
            # if( self.footUP ):
            #     ''' Raises the foot up moving it to the current x position of the CP '''
            #     self.taskCP.task.predictionTime.value = 138*0.005
            #     self.taskCP.task.predictedCapturePoint.recompute(self.sot.solution.time)
            #     goto6dPP(self.taskF, (self.taskCP.task.predictedCapturePoint.value[0],
            #                           self.taskCP.task.predictedCapturePoint.value[1],
            #                           0.105,0,0,0),
            #              (-0.3,0,0,0,0,0), 0.69/2, self.sot.solution.time)
            #     print 'UP'
