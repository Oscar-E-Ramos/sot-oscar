...
#      ContactSelect_new to handle the contacts in the dynamic sot
#      - There is no task for the arms (and they are kept backwards "by default")
#      - There is collision detection with a grid (the foot is modeled as a box)
#      - There is a 'grid' on the foot to 'discretize' the collision points
# ======================================================================================
...
from dynamic_graph.sot.oscar import SolverMotionReduced, FclBoxMeshCollision, FootCollisionGrid
...
cornersRF = ((0.13,-0.10,-0.10,0.13),(-0.0775,-0.0775,0.0575,0.0575),(-0.105,-0.105,-0.105,-0.105))
cornersLF = ((0.13,-0.10,-0.10,0.13),(0.0775,0.0775,-0.0575,-0.0575),(-0.105,-0.105,-0.105,-0.105))
arPcomr = matrix(( average(cornersRF[0]), average(cornersRF[1]), -0.105 + 0.012 )).T
alPcoml = matrix(( average(cornersLF[0]), average(cornersLF[1]), -0.105 + 0.012 )).T
...
# --- Grid for foot collision ----
sensorCloseness = 0.014

gridRf = FootCollisionGrid('gridRf')
gridRf.setGridStepSize(sensorCloseness)
gridRf.setFootCorners(cornersRF)
plug(fclRf.contactPoints, gridRf.contactPointsIN)
plug(dyn.rf, gridRf.footTransf)

gridLf = FootCollisionGrid('gridLf')
gridLf.setGridStepSize(sensorCloseness)
gridLf.setFootCorners(cornersLF)
plug(fclLf.contactPoints, gridLf.contactPointsIN)
plug(dyn.lf, gridLf.footTransf)
...
# for i in range(4):                                         # Hide the contact points
#     name = 'point'+str(i+1)
#     arM = bmat([ [mat(eye(3)),matrix(((cornersRF[0][i],),(cornersRF[1][i],),(cornersRF[2][i],)))], 
#                  [mat(zeros(3)),mat(1.)] ])
#     wM = matrixToTuple(matrix(dyn.rf.value)*arM) 
#     robot.viewer.updateElementConfig(name, [wM[0][3],wM[1][3],wM[2][3],0,0,0])
...
contactSelect.setCollisionDetection(gridRf, gridLf)
...
robot.before.addSignal('gridRf.contactPoints')
robot.before.addSignal('gridLf.contactPoints')
